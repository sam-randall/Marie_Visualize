...
def matrix_new(df,
    ...
    g[n] = [1, 0.9, 0.9] # washout red
    for i in range(0, 10): # black is high values.
...
def geoplot_slr(df,
    ...
    df = nullity_filter(df, filter=filter, n=n, p=p)
    df = nullity_sort(df, sort=sort)
    nullity = (df.notnull().sum(axis='columns') / df.shape[1]).tolist()
    tuples= [(lat[i],long[i],nullity[i]) for i in range(0,len(lat))]
    plt.scatter(lat, long, c=nullity)
    plt.show()
    return tuples

def geoplot(df,
            filter=None, n=0, p=0, sort=None,
            x=None, y=None, figsize=(25, 10), inline=True,
            by=None, cmap='YlGn', **kwargs):
